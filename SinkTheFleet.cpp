...
//
// Input:		safeChoice() from cin, firing coordinates from cin
//
// Output:		display to cout
//
...
//				01/12/17 MF completed v 1.0
//				01/17/17 MF completed v 1.01
//				01/31/17 JW completed v 1.02
//   
//----------------------------------------------------------------------------
int main(void)
{
	...
	do
	{
		...
		for (whichPlayer = 0; whichPlayer <= NUMPLAYERS; whichPlayer++)
		{
			if (whichPlayer == 0)
			{
				...
				header(cout);
				readFromFileChoice = 
	safeChoice("Player 1, Would you like to read starting grid from a file?", 
		'Y', 'N');
			...
			else if (whichPlayer == 1)
			{
				...
				header(cout);
				readFromFileChoice = 
	safeChoice("Player 2, Would you like to read starting grid from a file?",
		'Y', 'N');
				if (readFromFileChoice == 'N')
				{
					setships(game, gridSize, whichPlayer);
		...
		while (!gameOver)
		{
			do
			{
				...
				shipHit = game[!whichPlayer].m_gameGrid[0]
					[coord.m_row][coord.m_col];
				...
				if (shipHit != NOSHIP)
				{
					...
					{
						printGrid(cout, game
							[whichPlayer].m_gameGrid[1], gridSize);
					...
					{
						printGrid(cout, game[whichPlayer].m_gameGrid[1], 
							gridSize);
					...
					else
					{
						...
						//adjust grid for hit
						game[!whichPlayer].m_gameGrid[0]
							[coord.m_row][coord.m_col] = HIT;
						game[whichPlayer].m_gameGrid[1]
							[coord.m_row][coord.m_col] = HIT;
						printGrid(cout, game[whichPlayer]
							.m_gameGrid[1], gridSize);
						cout << "HIT" << endl;

						if (targetInfo.m_piecesLeft == 0)
						{
							cout << shipNames[targetInfo.m_name] << 
								" is destroyed." << endl;
						}
				...
				else
				{
					//adjust for miss
					game[!whichPlayer].m_gameGrid[0]
						[coord.m_row][coord.m_col] = MISSED;
					game[whichPlayer].m_gameGrid[1]
						[coord.m_row][coord.m_col] = MISSED;
					printGrid(cout, game[whichPlayer]
						.m_gameGrid[1], gridSize);
